...
export class BillingPlanModel extends BaseModel {
   ...
   */
  @Expose() addons: any // AdditionalResource;
  /**
   * Custom SMTP
   */
  ...
  constructor(data: Partial<BillingPlanModel | Document> = {}) {
    super(data);
  }
}


export class BillingPlanList extends BaseListModel {

  plans: BillingPlanModel[];

  constructor(partial: Partial<BillingPlanList | { plans: Document[] | { [key: string]: string }[] }>) {
    super();
    if (partial.plans) {
      this.plans = partial.plans.map((user: any) => new BillingPlanModel(user));
    }
