  ...
  async addMember(id: string, input: any) {
    ...
    if (input.userId) {
      ...
      if (input.email && invitee.email !== input.email) {
        throw new Exception(Exception.USER_ALREADY_EXISTS, 'Given userId and email don\'t match', 409)
      }
      if (input.phone && invitee.phone !== input.phone) {
        throw new Exception(Exception.USER_ALREADY_EXISTS, 'Given userId and phone don\'t match', 409)
      }
    } else if (input.email) {
      invitee = await this.userRepo.findOneBy({ email: input.email });
      if (invitee && input.phone && invitee.phone !== input.phone) {
        throw new Exception(Exception.USER_ALREADY_EXISTS, 'Given email and phone don\'t match', 409)
      }
    } else if (input.phone) {
      invitee = await this.userRepo.findOneBy({ phone: input.phone });
      if (invitee && input.email && invitee.email !== input.email) {
        throw new Exception(Exception.USER_ALREADY_EXISTS, 'Given phone and email don\'t match', 409)
      }
    }
