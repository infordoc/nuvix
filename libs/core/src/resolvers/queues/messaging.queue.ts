...
import {
    SMS, Push, Email,
    FCM, APNS,
    Telesign, TextMagic, Twilio, Vonage, Msg91,
    Mailgun, Sendgrid, SMTP,
    SMSAdapter, PushAdapter, EmailAdapter,
    Attachment, Priority
...
export class MessagingQueue extends Queue {
    ...
    constructor(
        ...
        @Inject(GET_DEVICE_FOR_PROJECT) private readonly getProjectDevice: GetProjectDeviceFn
    ) {
        super();
    }

    async process(job: Job<MessagingJobData, any, MessagingJob>, token?: string): Promise<any> {
        switch (job.name) {
            case MESSAGE_SEND_TYPE_EXTERNAL:
                ...
                try {
                    await this.sendExternalMessage(dbForProject, message, deviceForFiles, project, undefined as any);
                } finally {
    ...
    private async sendExternalMessage(
        ...
        if (topicIds.length > 0) {
            ...
            for (const topic of topics) {
                const targets = topic.getAttribute('targets').filter((target: Document) =>
        ...
        if (userIds.length > 0) {
            ...
            for (const user of users) {
                const targets = user.getAttribute('targets').filter((target: Document) =>
                    target.getAttribute('providerType') === providerType
                );
                allTargets.push(...targets);
            }
        ...
        if (allTargets.length === 0) {
            await dbForProject.updateDocument('messages', message.getId(), message.setAttributes({
        ...
        const providerPromises = Object.keys(identifiers).map(async (providerId) => {
            ...
            switch (provider.getAttribute('type')) {
                ...
                case MESSAGE_TYPE_EMAIL:
                    ...
                    break;
                default:
                    throw new Error('Provider with the requested ID is of the incorrect type');
            }
            ...
            const batchPromises = batches.map(async (batch) => {
                ...
                switch (provider.getAttribute('type')) {
                    ...
                    case MESSAGE_TYPE_PUSH:
                        ...
                        break;
                    case MESSAGE_TYPE_EMAIL:
                        data = await this.buildEmailMessage(dbForProject, messageData, provider, deviceForFiles, project);
                        break;
                    default:
                        throw new Error('Provider with the requested ID is of the incorrect type');
                }
                ...
                try {
                    ...
                    deliveryErrors.push(`Failed sending to targets with error: ${error.message}`);
                } finally {
